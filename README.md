...
- **凭证 OCR 自动填单**：拍照、选图或选择 PDF 后，端上识别（ML Kit 中文模型，离线）自动填入金额、日期与描述；支持数电电子发票（自动识别价税合计、开票日期、项目名称）
- 行程详情内置分类占比图，支出按日期分组
- 内置分类与自定义分类（可选图标）
...
- 配置 WebDAV，手动备份、查看备份和恢复；支持**每日自动备份**（每天首次打开应用时）
- **应用锁**：指纹/人脸/锁屏密码验证（BiometricPrompt），冷启动锁定，离开超过 2 分钟自动加锁
- 数据默认保存在本机，不依赖后端服务
- 深色模式完整适配
...
- Haze 1.7.2（玻璃拟态背景虚化）
- ML Kit text-recognition-chinese 16.0.1（端上凭证 OCR，APK 体积 +约 20MB）
- androidx.biometric 1.1.0（应用锁）
- Coil 3
